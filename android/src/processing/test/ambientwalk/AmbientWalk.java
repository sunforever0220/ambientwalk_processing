...
{       
       // start_time=millis();
        ...
        prev_index=0;
        offset1=261.63f;//Pitch:C4
        offset2=329.63f;//Pitch:E4
        offset3=392.00f;//Pitch:G4
        //audio setup*
        int freq =8000;//*
...
//peak detection with debouncing
 if(acc_magnitude.get(i)>acc_magnitude.get(i-1)&&acc_magnitude.get(i)>acc_magnitude.get(i+1)&&acc_magnitude.get(i)>acc_avg&&acc_avg>1.5f){
...
public void genTone(double freqOfTone_1, double freqOfTone_2)
 {
   ...
   for (int i = 0; i < numSamples; i++) {
     ...
     sample[i] = 0.05f*(Math.sin(2 * Math.PI * i / (gentonefreq/freqOfTone_1))+Math.sin(2 * Math.PI * i / (gentonefreq/freqOfTone_2)));
   }
 else if (i%4==1){
   sample[i]=0.05f*(Math.sin(2 * Math.PI * i / (gentonefreq/offset1))+Math.sin(2 * Math.PI * i / (gentonefreq/offset2))+Math.sin(2 * Math.PI * i / (gentonefreq/offset3)));
   
 }
   else if(i%4==2){
       //sample[i] = 4*(Math.sin(2 * Math.PI * i / (gentonefreq/freqOfTone_1))+Math.sin(6 * Math.PI * i / (gentonefreq/freqOfTone_1))/3+Math.sin(10 * Math.PI * i / (gentonefreq/freqOfTone_1))/5)/Math.PI;
     sample[i] = 0.05f*Math.sin(2 * Math.PI * i / (gentonefreq/freqOfTone_1));
   }
   else {
     sample[i] = 0.05f*Math.sin(2 * Math.PI * i / (gentonefreq/freqOfTone_2));
   }
}
